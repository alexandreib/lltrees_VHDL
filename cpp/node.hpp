...
node(int level, int id_node, int size) :  l_node(NULL), r_node(NULL), isleaf(true), level(level), id_node(id_node), l_size(0), r_size(0),  size(size), index_col(0), impurity(std::numeric_limits<T>::max()), threshold(0), leaf_value(0) {}
friend class tree<T>;   
